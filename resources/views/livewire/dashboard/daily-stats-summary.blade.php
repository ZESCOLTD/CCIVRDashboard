...
<div class="card">
    ...
    <div class="card-body table-responsive p-0">
                        ...
                        <td>
                            ...
                            {{-- @if ($item->manual_edit === 'yes')
                                <button class="btn btn-sm btn-outline-secondary ml-2"
                                    wire:click="edit('{{ $item->network }}')" data-toggle="modal"
                                    data-target="#editModal" title="Edit Manually">
                                    ✏️
                                </button>
                            @endif --}}
                        </td>
        ...
        <div class="mt-3 p-3 bg-light rounded">
            <h4>Analysis</h5>
                <h4>
                    In the last 24 hours, a total of <strong>{{ number_format($total) }}</strong> sessions were
                    recorded.
                    The highest activity was on
                    <strong style="color: {{ $topColor }}">{{ ucfirst($top->network) }}</strong>
                    with <strong style="color: {{ $topColor }}">{{ number_format($top->previous) }}</strong>
                    sessions,
                    while <strong style="color: {{ $leastColor }}">{{ ucfirst($least->network) }}</strong>
                    had the lowest at <strong
                        style="color: {{ $leastColor }}">{{ number_format($least->previous) }}</strong> sessions.
                </h4>
        </div>
